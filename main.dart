...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadData() async {
    ...
    if (refsRaw == null) {
      ...
      await prefs.setString(
          kRefEventsKey, jsonEncode(_refs.map((e) => e.toJson()).toList()));
    } else {
  ...
  Future<void> _openReferenceTest() async {
    await Navigator.of(context)
        .push(MaterialPageRoute(builder: (_) => ReferenceTestPage(refs: _refs)));
    await _loadData();
  }
  ...
  Future<void> _openAddEvent() async {
    final x = computeX();
    await Navigator.of(context)
        .push(MaterialPageRoute(builder: (_) => AddEventPage(maxScale: x)));
    await _loadData();
  }

  Future<void> _openStats() async {
    await Navigator.of(context)
        .push(MaterialPageRoute(builder: (_) => StatsPage()));
    await _loadData();
  }
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ElevatedButton(
                onPressed: _openReferenceTest,
                child: Text('Пройти тест эталонов')),
            SizedBox(height: 12),
            ElevatedButton(
                onPressed: _openAddEvent,
                child: Text('Добавить событие дня')),
            SizedBox(height: 12),
            ElevatedButton(
                onPressed: _openStats, child: Text('Статистика')),
            SizedBox(height: 24),
            Text('Текущий масштаб x = ${computeX().toStringAsFixed(1)}',
                style: TextStyle(fontSize: 16)),
            SizedBox(height: 8),
            Text('Эталонные события (и ваши оценки):',
...
class _ReferenceTestPageState extends State<ReferenceTestPage> {
  ...
  Future<void> _save() async {
    ...
    for (int i = 0; i < widget.refs.length; i++)
      widget.refs[i].rating = _ratings[i];
    await prefs.setString(
        kRefEventsKey, jsonEncode(widget.refs.map((e) => e.toJson()).toList()));
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 8),
                        Text(
                            'Оцените это событие по вашей шкале (от -100 до 100):'),
                        Slider(
                          min: -100,
                          max: 100,
                          divisions: 200,
                        ...
                        Align(
                            ...
                            child: Text(_ratings[i].toStringAsFixed(0))),
                      ],
                    ),
                  ),
                );
...
class _StatsPageState extends State<StatsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    // Helper: get date string for x-axis label
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            Text('F(t) = S — кумулятивная сумма оценок по дням',
                style: TextStyle(fontWeight: FontWeight.bold)),
            SizedBox(height: 12),
            Expanded(
              child: spots.isEmpty
                              ...
                              sideTitles: SideTitles(
                                ...
                                getTitlesWidget: (value, meta) {
                                  ...
                                  if (index < 0 || index >= spots.length)
                                    return const Text('');
                                  return Text(_getDateLabel(index));
                                },
                              ),
                            ),
